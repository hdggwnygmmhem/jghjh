...
cmd({
    pattern: "storyanime",
    alias: ["storyanime", "animestory", "storianime", "animestoria"],
    desc: "Get random anime story video",
    category: "anime",
    react: "🌸",
    filename: __filename,
    limit: true,
...
async (conn, mek, m, { from, reply }) => {
    try {
        ...
        console.error("Error in storyanime command:", e);
        // ❌ React - error
        await conn.sendMessage(from, { react: { text: '❌', key: m.key } });
        await reply(`❌ Error fetching story anime: ${e.message}`);
    }
});
