...
/**
 * Fetch Audio Download Link using Vajira API with Safe String Parsing
 */
async function fetchAudioData(url, retries = 2) {
  try {
    ...
    if (data && (data.status === true || data.status === 200)) {
      ...
      // Extract the download link safely
      let finalUrl = res.download || res.dl || res.mp3 || res.url;
      
      // Agar URL object ki form mein ho toh usay string mein convert karna
    ...
    throw new Error("Vajira API failed to return a valid audio link string.");
  } catch (error) {
    if (retries > 0) {
